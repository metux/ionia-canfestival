...
void timer_notify(sigval_t val)
{
	...
	EnterMutex();
	gettimeofday(&last_sig,NULL);   // here, o/w a canDispatch (mutexed) may use a wrong getElapsedTime value if a SetAlarm() happens just before the EnterMutex
	TimeDispatch();
	LeaveMutex();
	//printf("getCurrentTime() return=%u %u\n", last_sig.tv_sec, last_sig.tv_usec);
}
