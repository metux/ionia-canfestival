...
void ConsumerHearbeatAlarm(CO_Data* d, UNS32 id)
{
    ...
    //d->ConsumerHeartBeatTimers[id]=TIMER_NONE;  // CHB see restart supervision
    
    // printf("%s HB notification Id=%d nodeId=%d\n", __func__,id, nodeId);
    /* set node state */
    d->NMTable[nodeId] = Disconnected;
    
    /* Restart supervision for polling with eCLR CHB */
    TIMEVAL time = ( (d->ConsumerHeartbeatEntries[id]) & (UNS32)0x0000FFFF ) ;
    DelAlarm(d->ConsumerHeartBeatTimers[id]);
    
    d->ConsumerHeartBeatTimers[id]              \
       = SetAlarm(                              \
           d,                                   \
           id,                                  \
       ...
       */
    {
        ...
        */
        ...
        if( d->NMTable[nodeId] != Unknown_state ) {
            //printf("HB nodeID=%d state=%d  arrived\n", nodeId, d->NMTable[nodeId]);
            ...
            for( index = (UNS8)0x00; index < *d->ConsumerHeartbeatCount; index++ )
            {
                ...
                // printf("ConsumerHeartBeat_nodeId=%d\n",ConsummerHeartBeat_nodeId);
                if ( nodeId == ConsummerHeartBeat_nodeId )
                {
                    ...
                    // printf("Restart hb supervison nodeId=%d\n", ConsummerHeartBeat_nodeId); 
...
UNS32 OnHeartbeatProducerUpdate(CO_Data* d, const indextable * unsused_indextable, UNS8 unsused_bSubindex)
{
  heartbeatStop(d);
  heartbeatInit(d);
  if (*d->ProducerHeartBeatTime) d->CurrentCommunicationState.csHeartbeat = 1;    // sets if ProducerHeartBeatTime != 0//fj
  return 0;
}
...
void heartbeatInit(CO_Data* d)
{
    ...
    //diag_printf("HB Init()  .................  ConsumerHeartbeatCount()=%d\n",*d->ConsumerHeartbeatCount); CHB
    ...
    for( index = (UNS8)0x00; index < *d->ConsumerHeartbeatCount; index++ )
    {
        ...
        if ( time )
        {
            ...
            /* diag_printf("ConsumerHeartBeatTimers[%d]  time=%d\n",     \ */
            /*        index,                                    \ */
            /*        time);  */
        }
    }
