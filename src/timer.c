...
//	        printf("set row_number %d  elapsed_time %llu callback %p value %llu period %llu\n", row_number, elapsed_time, callback, value, period);//fj
...
//    printf("real_total_sleep_time %llu  total_sleep_time %llu  overrun %d  \n", real_total_sleep_time, total_sleep_time, overrun);//fj
	...
	for(i=0, row = timers; i <= last_timer_raw; i++, row++)
	{
		...
		{
//	        printf("i %d row->val %llu  row->interval %llu  row->callback %p  ", i, row->val, row->interval, row->callback);//fj
...
//            printf("do %p %llu %llu\n", row->callback, row->val, row->interval);//fj
			if(row->callback)
