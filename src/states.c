...
extern UNS32 ticks_last_timestamp;

UNS32 ticks_last_timestamp;
...
void canDispatch(CO_Data* d, Message *m)
{
	...
	switch(cob_id >> 7)
	{
		...
		case TIME_STAMP:
        {
        	UNS32 errorCode;
        	UNS32 data;
        	const UNS16 odIndex = 0x1012;
        	ODCallback_t *Callback;
        	const indextable *ptrTable;

        	ptrTable =(*d->scanIndexOD)(odIndex, &errorCode, &Callback);
        	if (errorCode != OD_SUCCESSFUL) {
        		break;
        	  }
        	data = *(UNS32*)ptrTable->pSubindex[0].pObject;
        	if(data & 0x80000000) {
        		UNS16 can_days = (m->data[5]<<8) + m->data[4];
        		UNS32 can_ms = (((((m->data[3]<<8) + m->data[2])<<8) + m->data[1])<<8) + m->data[0];
        		time_t t;
        		t = (can_days + 5113) * 86400 + can_ms / 1000;  // 5113 days from 1.1.70 to 1.1.84
        		stime (&t);  // must be superuser? well, it works!
        		ticks_last_timestamp = time(NULL);
//              // - fj - for test only
//              time_t t1 = time(NULL);
//              printf("ms: 0x%08x; time set to %s", can_ms, ctime(&t1));
//              // -
        	}
		    break;
        }		    
...
//printf("bef %d %d\n", newCommunicationState->csHeartbeat, d->CurrentCommunicationState.csHeartbeat);//fj
