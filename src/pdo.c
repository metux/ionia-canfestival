...
proceedPDO (CO_Data * d, Message * m)
{
  ...
  numPdo = 0;
  numMap = 0;
  UNS32 RTR_MASK = 0xBFFFFFFF;  /* ignor rtr bit in RPDO */
        ...
        while (offsetObjdict <= lastIndex)
          {
            ...
            switch (status)
              ...
              case state2:
                 ...
                 */
                if ((*pwCobId & RTR_MASK) == UNS16_LE(m->cob_id))
                  {
                    /* The cobId is recognized */
                    status = state4;
                    MSG_WAR (0x3936, "cobId found at index ",
                             0x1400 + numPdo);
                    /* diag_printf("FOUND: d=0x%X, pwCobId=0x%X,    \
                     *              m->cob_iD=0x%X\n",              \
                     *              d,                              \
                     *              (*pwCobId & RTR_MASK),          \
                     *              UNS16_LE(m->cob_id));           */
                ...
                else
                  {
                    ...
                    offsetObjdict++;
                    status = state2;
                    /* diag_printf("NOT FOUND: d=0x%X, pwCobId=0x%X, \
                     *              m->cob_iD=0x%X\n",               \
                     *              d,                               \
                     *              (*pwCobId & RTR_MASK),           \
                     *              UNS16_LE(m->cob_id));            */
...
PDOInit (CO_Data * d)
{
  ...
  /* Trigger a non-sync event, otherwise event-based TPDOs are not sent */
  _sendPDOevent (d, 0);
}
